			...
			onTheSchedulePage : {
				...
				actions : jQuery.extend({
						...
						return this.waitFor(createWaitForItemAtPosition({
							...
							success : function (oTableItem) {
								...
								this.getContext().currentItem = {
									...
									id: oBindingContext.getProperty("LessonTime"),
									name: oBindingContext.getProperty("LessonName")
								};
							}
						}));
					},
					...
					iSearchForValueWithActions : function (aActions) {
						return this.waitFor({
							...
							viewName : sViewName,
							actions: aActions,
							errorMessage : "Failed to find search field in Schedule view.'"
						});
					},
				...
				assertions: jQuery.extend({
					...
					theTableShouldHaveAllEntries : function () {
						...
						// retrieve all Lessons to be able to check for the total amount
							...
							success : function (oTable) {
								...
								this.waitFor({
									...
									matchers : function (oPage) {
										var sExpectedText = oPage.getModel("i18n").getResourceBundle().getText("scheduleTableTitleCount", [iObjectCount]);
					...
					iShouldSeeTheScheduleViewsBusyIndicator : function () {
						return this.waitFor({
							id : "page",
							viewName : sViewName,
							success : function (oPage) {
								Opa5.assert.ok(oPage.getParent().getBusy(), "The schedule view is busy");
							},
							errorMessage : "The schedule view is not busy"
						});
					},

					iShouldSeeTheScheduleTableBusyIndicator : function () {
						return this.waitFor({
							id : "table",
							viewName : sViewName,
							matchers : function (oTable) {
							...
							success : function () {
								Opa5.assert.ok(true, "The schedule table is busy");
							},
							errorMessage : "The schedule table is not busy"
						});
					},
					...
					iShouldSeeTheNoDataTextForNoSearchResults : function () {
						return this.waitFor({
							...
							viewName : sViewName,
							success : function (oTable) {
								Opa5.assert.strictEqual(oTable.getNoDataText(), oTable.getModel("i18n").getProperty("scheduleNoDataWithSearchText"), "the table should show the no data text for search");
							},
